...
class Index(BaseObject):
    ...
    @property
    def date(self) -> date:
        """Returns a Python date object based on its period."""
        return date(self.year, self.period.month, 1)


class Series(BaseObject):
    """A Consumer Price Index data series generated by the U.S. Bureau of Labor Statistics.
    ...
    def latest_month(self) -> date:
        q = queryone(
            "SELECT max(date) FROM 'indexes' WHERE series=? AND period_type='monthly'",
            (self.id,),
        )
        dt_str = q["max(date)"]
    ...
    def latest_year(self) -> int:
        q = queryone(
            "SELECT max(year) FROM 'indexes' WHERE series=? AND period_type='annual'",
            (self.id,),
        )
        return int(q["max(year)"])

    def get_index_by_date(self, dt: date, period_type: str = "annual") -> Index:
        """Retrieve the index value for a specific date and period type.

        Args:
            date (date): The date to retrieve the index for.
            period_type (str): The type of period to retrieve the index for.

        Returns:
            Index: The index value for the date and period type.
        """
        # Check if the indexes is already in the cache
        try:
            return self._indexes[period_type][dt]
        except KeyError:
            # If it's not, we will query it
            pass

        # Query the database for the index we want
        d = queryone(
            "SELECT * FROM 'indexes' WHERE series=? AND date=? AND period_type=?",
            (self.id, dt, period_type),
        )
        ...
        obj = Index(
            series_id=self.id,
            year=dt.year,
            period=Period.get_by_id(d["period"]),
            value=float(d["value"]),
        )

        # Cache it by period_type and date
        self._indexes[period_type][dt] = obj
